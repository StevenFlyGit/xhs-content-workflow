...
describe('generation state', () => {
  it('keeps both generated modes valid when only the display mode changes', () => {
    const input = { name: '项目', eventName: '活动', eventType: 'Conference', originalText: '原文' }
    const record = {
      ...input,
      outputMode: 'summary',
    ...
    expect(hasCurrentGeneration({ ...record, outputMode: 'card' }, 'card')).toBe(true)
  })

  it('invalidates generated content after the source text changes', () => {
    const input = { name: '项目', eventName: '活动', eventType: 'Conference', originalText: '旧原文' }
    expect(hasCurrentGeneration({
      ...input,
      originalText: '新原文',
